...
- 2026-09-18 (P2-PARSE-012): pdfjs-dist in-process (no worker service, no
  Python); body size is document-wide with ties breaking smaller (a 2-item
  page must not elect its title — found via failing fixture, fixed, pinned);
  section stack carries across pages; reading order from glyph positions, not
  hasEOL; fixtures generated by pdf-lib (reviewable, no binaries); encrypted
